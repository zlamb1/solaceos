...
ARCHSRCS := io endian atomic spinlock console
SRCSRCS  := kmain string panic console vga print kprint
MMSRCS   := palloc

KRNSRCS := $(patsubst %,$(ARCHDIR)/%.c,$(ARCHSRCS)) \
	       $(patsubst %,$(SRCDIR)/%.c,$(SRCSRCS)) \
		   $(patsubst %,$(SRCDIR)/mm/%.c,$(MMSRCS))
